...
def construct_decoder(Config) -> Decoder:
    name = Config().server.autos.method_name
    size = Config().clients.total_clients
    info(f'Construct Decoder with method {name}...')
    if name == 'rnn':
        return RNNDecoder(
            layers=Config().server.autos.decoder_layers,
            vocab_size=size + 1,
            hidden_size=Config().server.autos.decoder_hidden_size,
            dropout=Config().server.autos.decoder_dropout,
            length=size,
            gpu=Config().server.autos.gpu
        )
    else:
        assert False
